...
| Location fallback (only when GPS is off, denied, or fails) | ipapi.co | your IP address (used by the service to look up a city) and ordinary request metadata — no coordinates, no request body | work out an approximate location without GPS, so prayer times, Qibla, nearby mosques and the other location-based features below still have somewhere to start from — sent automatically only as a last resort, and at most once every 10 minutes while GPS stays unavailable |
...
| Islamic Videos | `d1.islamhouse.com` (IslamHouse.com's own file host) | a thumbnail image (automatically, to show the video card on Home and the list in *More → Videos*), and the full video (only when you tap play) | show and stream Islamic videos — everything is fetched from IslamHouse's own host, not from us |
| Live radio | `mp3quran.net` for the station list; whichever host the station you tap actually streams from — a real broadcaster's own server, not mp3quran.net or us | the `language` filter to list stations; nothing beyond that to play one | show and stream live radio — sent only when you open *More → Radio*, and only when you tap a station to play it |
...
**When GPS gives nothing — permission denied, Location Services off, or the fix fails — the app asks ipapi.co to place you from your IP address instead**, so prayer times, Qibla and the other location-based features below still work rather than showing an empty screen. This needs no permission and no prompt, because it uses no location hardware at all; it only ever runs after a device-location attempt has already failed and no saved city is active, and at most once every 10 minutes for as long as GPS stays unavailable — it doesn't keep re-asking on every app open. It is as approximate as your internet connection allows (typically city-level, sometimes less precise on mobile data or a VPN), and until GPS succeeds it can be treated by the app the same as an ordinary device fix, including by the background prayer-alert refresh described below. The only way to stop it running is the same one that stops everything else here: enter a city by hand in *More → Locations* instead of using "Current location".
