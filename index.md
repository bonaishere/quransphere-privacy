...
| Al-Azhar Videos | `youtube.com` — opened in the YouTube app or your browser when you tap a channel in *More → Videos*. The app itself contacts nothing; it only hands the link to YouTube, which is then governed by Google's own privacy policy | nothing from us — the link is a plain channel address | open an official Al-Azhar or Dar al-Iftaa channel on YouTube |
| Live radio | `mp3quran.net` for the station list; `stream.radiojar.com` for the one bundled Egyptian station (Cairo Quran Radio); otherwise whichever host the station you tap actually streams from — a real broadcaster's own server, not us | the `language` filter to list stations; nothing beyond that to play one | show and stream live radio — sent only when you open *More → Radio*, and only when you tap a station to play it |
| Fiqh & Fatwa | `dar-alifta.org` — opened in your browser only if you tap the source link at the bottom of a ruling. The rulings themselves are stored inside the app; reading them needs no network | nothing from us | let you open the original fatwa on Dar al-Iftaa's own site |
